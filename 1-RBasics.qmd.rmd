...
```{r}
paste("Hello", "My name is Indira")
```

(@) Can you explain what happened in 2-3 sentences? 
What I believed happened is that the two "", became one. Since you are not able to add using the addition sign, one must separate them both, before combining them. 
> 
...
It produces an output of whether or not the function is 'TRUE' or 'FALSE'.
>
...
```{r}
My_favorite_color <- 13
```

(@) Can you assign a string to a variable?

```{r}
string1 <- 'My name is Indira'
```
...
> The numbers in vectors got multiplied by the value which was my.numbers. So 2, 4, 6, 8, got multiplied by 2 doubling each number in the second code which resulted in 4, 8, 12, 16.
...
>The output [1] 12 is listing the third number in the list. The original c(2, 4, 6, 8) the third number is 6 and it is being multiplied by 2 which gave the output of [1] 12.

(@) You try, create a list of five items and store it in a descriptive variable. 
```{r}
mycolor <- c("Pink", "Blue", "Green", "Orange", "Purple")
```
...
>It goes directly to the R Help guide that lays out the description of each function (ceiling, floor, trunc, round, and signif).
...
>Function (x,y) the first row is being multiplied by "2" which are the two values being used. "Z" being in the third column in the data frame, x being the first column multiplying by 2 (x,y) + y. (I might've overthinked it)
...
>The scope of the dataset is only one data frame wit 14 variables and 6098 observations. Also can see that it was last updated in 2022. 
...
>In order to create a new function, I had to customize the number of rows displayed. In the first function, it automatically gives you the default output of 6 rows. To make it show the first ten I had to customize the argument to the number we had to use which was 10. For the last 6 rows of data, I had to change the input to "tail", because I want the output to only be the last 6 rows of data.
...
>gayguides is the dataset, while the $ sign allows us to get only a snippet of data and it is sorted by the 'title' and that $ sign is telling it to pull the 500th item in the list. 
...
> I saw the first 6 rows of data of the almshouse admissions. I also saw the descriptions of the columns which included clerk, reason, year discharged, and month discharged. 




CONGRATS! You've completed your first worksheet for Digital Methods I. That wasn't so bad, right? 
