...
> In order to save it to another data frame, I had to create a new data frme using the existing data from gay guides. We use library(dplyr) in order to manipulate the already created dataset. Then I had to use the select() function to only extract city and state from the data. 
...
```{r}
gayguides$city_state <-c('location')
...
statepopulations %>%
    ...
    select('state')
```
I think the answer is Alabama, I also maybe misinterpreting the question, but code runs. 
...
```
Fourteen southern states in total.
...
gayguides <- left_join(regions, by=locations, == 1980)

head(gayguides) 
```

(@) Explain what you did above. What variable did you join by and why? What results?
Six locations appear in the Mountain region in 1980. I knew that I had to join the two data frames together using the left_join. The variable I had to join by is locations because I only wanted that in the output, and only equal to the year of 1980. 
...
```{r}
gayguides %>%
  data.frame(gayguides)
  cbind.data.frame(locations, region)
...
> This code combined columns that were standing alone into four consecutive rows that categorize the data better, than before it only produced the csv file with multiple columns making it harder to manipulate the data. That's the importance of 'pivot_longer()'.
...
```
I didn't see the 'judges' dataset, so I'm using the charlestonManumissions dataset. The question I am asking is how popular is the motive 'moral' is? In the year of 1777, since that year appears frequently. 
...
charlestonManumissions %>%
  ...
  summarize(count = n())
```
In order to figure out this question I had to access the dataset first. Then I had to select what I wanted to figure out which is 'Motive'. From there I had to figure out what motive I was looking for which was 'moral' with the year of 1777. From there I had to summarize the count.

(@) Now ask a question of the `gayguides` data (or another dataset of your choice). What is the question you are asking? 
```{r}

```
The next question I am asking is of the almshouse admissions. What year was the most people were admitted? The answer I got was the year 1801.

(@) Now write some code to address that question. Comment the code with notes that explain your thinking as you go. Use functions like select(), filter(), etc to pair down your dataset and reshape it to address your question. 
```{r}
almshouse_admissions %>%
  select(Year.Admitted) %>%
  group_by(Year.Admitted) %>%
  summarize(count = n())
  
```
At first the code wouldn't run because I didn't have select(), instead I was trying to filter it at first, which was wrong. I wanted to group the output with the year admitted only. The summarize was the count of how many indiciduals were admitted during a given year.
...
#edit at end so I can comment -AR
