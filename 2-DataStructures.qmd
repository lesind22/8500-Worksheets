...
>The third number in the list "2" is to be excluded, as well as the first number of the sequence which is "15.4". So the final result "[1] 25.00 8.35 5.00 383.00 10.20" omit those numbers because of this negative integer. 
...
> In line 62 the vector is only using five numbers out of the seven. So logically the output if "TRUE" or "False", not returning what was in the original vector. In the sequence 15.40, 2.00, 8.35, and 383.00 all have different retruning results reflecting the "TRUE" or "FALSE" outputs. In line 67, I believe that it is saying that "x" is less than 7 which represents the overall numbers in the vector. 
...
> The vector is separating names from letters, and it is also asking to show the first number in the set A-D. The remaining numbers do not have a letter to match, except the return values of what was given in the original vector set (383.00 and 10.20)
#### Matrices
...
> The first subset, shows only rows 2 and 3 in the output. Versus [1:3,] is only showing rows 1 through 3 with no specifying the only rows that want to be shown, not the entire dataset. 
...
for (name in 1:length(charlestonManumissions$Name)) {
  if (charlestonManumissions$Name[na] =< 30) {
    print(charlestonManumissions$Name[na])
  }
}
charlestonManumissions[1:10, ]
```

> In this dataset (it's loading as a csv. not too sure why) but I chose to subset the first ten rows of data, which showed 4 different categories. The result shows the first 10 individuals, their name, race, owner, and their motive which I think is a critical category to have. It also showed the date, volume, and page that the information is located on, but specifically for the first 10 rows. 
...
>This line of code is specific for getting to the cities in the gayguides dataset. It also displays the cities between lines 100-110, not the entire dataset. 
...
```{r}
presidents <- c(Washington, Regan, Roosevelt, Bush, Kennedy)
if (presidents < 5) {
print ("it is FALSE.")
...
> I had to extract the dataset from the library. From there I began to build the "if" statement. The max year is less than 1990 because we don't want to go beyond that year. The $ is accessing other data, within the dataset. Also asking it to print "FALSE" if they are any years over 1990, and displaying in the statement whether or not if the condition is "TRUE", then the output will print "TRUE". 
...
```{r}
presidentslength() <- c("George Washington", "John Adams", "Thomas Jefferson")
...
> It works because the listed presidents, while length () is returning the number of presidents. We use it on  'presidents' because it will return the total number of presidents that are in the string. 
...
```{r}
my_classes <- c("HIST 8500", "HIST 8510", "HIST 6930")
for i= 1:("I am enrolled in") {
  else ("I am not enrolled in")
  print ("FALSE")
}
```
...
> The reason why the loop only printed 3 and 4 is because ">=" 5 is greater than 3 and 4. With next its skipping the other columns and only printing 3 and 4.
...
```{r}
if(statepopulations$)
  if(statepopulations$1800 [i] > 25000){
    next
  }
  if (statepopulations$1800 [i] < 20000){
    next
  }
...
```{r}
gayguides <- generate_cities(Greenville)
while (gayguidesdata$location$Greenville)
  print(greenville)
```
...
count_characters <- function(text) {
  ...
  paste("There are", character_count, "characters in that string")
} 


my_string <- "Hello, world!"
result <- count_characters(my_string)
print(result)  # Output: "There are 13 characters in that string" 
```

(@) Reflect on the process of building the above function. How did you go about figuring this out? 

>In order for it to output how many characters are in the string, I had to call it. "Count_characters" is taking a single argument, which would become a string once the characters are counted in. The paste function is also important because it is combining the string "there are" which calculates into "character count", into a single output string, returned by the function " [1] There are 13 characters in that string".
...
>Because it isn't calling on anything in the ordered list to produce the output of (60).
...
gayguides <- get_state_data("South Carolina",1989) 
print(gay_guides_data1989) 
...
names$BostonWomenVoters$occupation

occupation$(BostenWomenVoters)
